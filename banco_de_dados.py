...
.
        """
        query = "SELECT id, descricao, concluida, em_andamento, atrasada FROM tarefas WHERE usuario_id = ?"
        c = bd.cursor()
    ...
    @staticmethod
    def adicionar_tarefa(bd:sqlite3.Connection, usuario_id:int, descricao:str)-> None:
        """
        Adiciona uma nova tarefa na tabela de tarefas.

        """
        query = "INSERT INTO tarefas (usuario_id, descricao, concluida, em_andamento, atrasada) VALUES (?, ?, ?, ?, ?)"
        c = bd.cursor()
        c.execute(query, (usuario_id, descricao, False, False, False))
        bd.commit()
        c.close()

    @staticmethod
    def atualizar_tarefa(bd:sqlite3.Connection, tarefa_id:int, descricao:str, concluida:bool, em_andamento:bool, atrasada:bool)-> None:
        """
        Atualiza uma tarefa que já existe na tabela 'tarefas'.

        """
        query = "UPDATE tarefas SET descricao = ?, concluida = ?, em_andamento = ?, atrasada = ? WHERE id = ?"
        c = bd.cursor()
        c.execute(query, (descricao, concluida, em_andamento, atrasada, tarefa_id))
        bd.commit()
        c.close()
    ...
    def verificar_usuario(bd:sqlite3.Connection, usuario:str)-> tuple:
        ...
        c.close()
        print(f"Função verifica_usuario {usuario_encontrado}")
